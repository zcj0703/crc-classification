...
class MyCustomModel(object):
  ...
  def forward_pass(self, x, input_data_format='channels_last'):
    ...
    # Use basic (non-bottleneck) block and ResNet V1.
    res_func = self._residual_v1

    x = self._conv(x, 7, 64, 1)
    ...
    x = self._avg_pool(x, 2, 2)
    x = res_func(x, 3, 512, 1024, 2)
    x = res_func(x, 3, 1024, 1024, 1)
    x = res_func(x, 3, 1024, 1024, 1)
    x = self._batch_norm(x)

    x = res_func(x, 3, 1024, 2048, 2)
    x = res_func(x, 3, 2048, 2048, 1)
    x = res_func(x, 3, 2048, 2048, 1)
    x = self._batch_norm(x)

    x = tf.contrib.layers.flatten(x)
    x = self._fully_connected_relu(x, 1024)
    x = self._fully_connected_relu(x, 1024)
  ...
  def _dropout(self, x, rate):
    with tf.name_scope('dropout') as name_scope:
      return tf.layers.dropout(x, rate, training=self._is_training)
