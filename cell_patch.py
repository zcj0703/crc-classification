...
csv_path='/hpcstore01/dp3/tcga_normal/tcga_normal.csv'
output='/mnt/nfs/tcga_output2/'
patchsize=256


def cut(filename):
    slide = openslide.OpenSlide(dir_path+filename+'.svs')
    ...
    for contours in contours0:
        ...
        while yyy<yy+hh:
            while xxx<xx+ww:
                ...
                if whitearea<0.75:
                    cv2.imwrite(output+str(name_img)+'_'+str(xxx)+'_'+str(yyy)+'.tif', patch)
